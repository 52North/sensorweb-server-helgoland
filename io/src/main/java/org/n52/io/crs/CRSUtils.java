...
public final class CRSUtils {
    ...
    /**
     * Creates a GeoJSON representation of the given point. Adds a named
     * <code>crs</code> member if it is different to the internally used CRS:84.
     *
    ...
    /**
     * Creates a 2D point geometry with respect to its height and given
     * reference system.
     *
    ...
    /**
     * Extracts the SRS number of the incoming SRS definition string. This can
     * be either an HTTP URL (like
     * <code>http://www.opengis.net/def/crs/EPSG/0/4326</code>) or a URN (like
     * <code>urn:ogc:def:crs:EPSG::31466</code>).
     *
     * @param srs the SRS definition string, either as URL
     * ('<code>/</code>'-separated) or as URN ('<code>:</code> '-separated).
     ...
     * @param srs the SRS definition string, either as URL
     * ('<code>/</code>'-separated) or as URN ('<code>:</code> '-separated).
     * @return SRS string in the form of for example 'EPSG:4326' or
     * 'EPSG:31466'.
     */
    ...
    /**
     * Transforms a given geometry from a given reference to inner reference,
     * which is WGS84 (CRS:84).
     *
     * @param geometry the geometry to transform.
     * @param srcFrame the CRS authority code the given point is referenced in.
     * @return a geometry referenced in WGS84
     * @throws FactoryException if the creation of
     * {@link CoordinateReferenceSystem} fails or no appropriate
    ...
    /**
     * Transforms a given geometry from its inner reference (which is WGS84
     * (CRS:84)) to a given reference.
     *
     * @param geometry the geometry to transform.
     * @param destFrame the CRS authority code the given point shall be
     * transformed to.
    ...
    /**
     * Transforms a given geometry from a given reference to a destinated
     * reference.
     *
     * @param geometry the geometry to transform.
    ...
    /**
     * Indicates if the given reference frame has switched axes compared to the
     * inner default (lon/lat).
     *
     * @param outer the given reference frame code to check.
     * @return <code>true</code> if axes order is switched compared to the inner
     * default.
    ...
    /**
     * Gets the propert coordinate reference system defined for the given
     * authority code. If no matching CRS could be found the default
     * {@link #internCrs} is being returned.
     *
     * @param authorityCode the CRS code, like <code>EPSG:4326</code> or
     * <code>CRS:84</code>.
     * @return the CRS instance for the given code or {@link #internCrs} if
     * either no matching CRS could be found or an error occured during
     * resolving code.
     ...
     * @return <code>true</code> if the first axes of both given CRS do not
     * point in the same direction, <code>false</code> otherwise.
     */
    ...
    /**
     * Creates an {@link CRSUtils} which offers assistance when doing spatial
     * opererations. Strict means that all CRS defined with lat/lon axis
     * ordering will be handled as defined.
     *
     * @return creates a reference helper which (strictly) handles referencing
     * operations.
    ...
    /**
     * Creates a {@link CRSUtils} which offers assistance when doing spatial
     * opererations. Forcing XY means that CRS axis ordering is considered
     * lon/lat ordering, even if defined lat/lon.
     *
     * @return creates a reference helper which (strictly) handles referencing
     * operations.
     * @throws IllegalStateException if decoding default CRS fails.
     ...
     * opererations.
     *
     * @param hints Some Geotools {@link Hints} which set behavior and special
     * considerations regarding to the spatial operations.
