...
import org.n52.io.response.series.dwd.DwdAlertObservationSeriesOutput;
import org.n52.io.response.series.dwd.DwdAlertObservationValue;
import org.n52.io.response.series.dwd.DwdAlertValue;
...
public class SeriesOutputAdapter extends AbstractOuputAdapter<SeriesMetadataOutput> {

    private final AlertStore store;

    private final UrlHelper urlHelper = new UrlHelper();

    public SeriesOutputAdapter(AlertStore store, ServiceInfo serviceInfo) {
        super(serviceInfo);
        this.store = store;
    }

    private OutputCollection<SeriesMetadataOutput> createOutputCollection() {
        return new OutputCollection<SeriesMetadataOutput>() {
            @Override
    ...
    public OutputCollection<SeriesMetadataOutput> getExpandedParameters(IoParameters query) {
        OutputCollection<SeriesMetadataOutput> outputCollection = createOutputCollection();
    ...
    private Alert getAlert(String id) {
        List<String> parseId = parseId(ObservationType.extractId(id));
    ...
    private DwdAlertObservationSeriesOutput createCondensed(Alert alert, WarnCell warnCell, IoParameters query) {
        DwdAlertObservationSeriesOutput result = new DwdAlertObservationSeriesOutput();
    ...
    private SeriesMetadataOutput createExpanded(Alert alert, WarnCell warnCell, IoParameters query) {
        DwdAlertObservationSeriesOutput result = createCondensed(alert, warnCell, query);
        result.setSeriesParameters(getSeriesParameters(alert, warnCell, query));
        DwdAlertObservationValue value = createValue(alert);
        result.setFirstValue(value);
        result.setLastValue(value);
        return result;
    }

    private DwdAlertObservationValue createValue(Alert alert) {
        DwdAlertObservationValue value = new DwdAlertObservationValue();
        value.setTimestamp(store.getLastKnownAlertTime().getMillis());
        value.setValue(createAlertValue(alert));
        return value;
    }

    private DwdAlertValue createAlertValue(Alert alert) {
    ...
    private void checkForHref(SeriesMetadataOutput result, IoParameters parameters) {
