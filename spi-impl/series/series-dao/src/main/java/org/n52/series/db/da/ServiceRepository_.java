...
public class ServiceRepository_ extends SessionAwareRepository implements OutputAssembler<ServiceOutput> {

    private static final Logger LOGGER = LoggerFactory.getLogger(ServiceRepository_.class);
...
//        final ServiceSearchResult result = new ServiceSearchResult(serviceEntity.getServiceId(), serviceEntity.getServiceDescription());
//        String queryString = DbQuery.createFrom(parameters).getSearchTerm();
//        return serviceEntity.getServiceDescription().contains(queryString)
...
//                : Collections.<SearchResult>emptyList();
    ...
    public ServiceEntity getEntityInstance(String id, DbQuery parameters) throws DataAccessException {
    ...
    private ServiceOutput getExpandedService(DbQuery parameters) {
        ...
        if (filterResolver.shallBehaveBackwardsCompatible()) {
            ...
            service.setType(serviceEntity.getType() == null
                    ? "Thin DB access layer service."
                    : serviceEntity.getType());
            service.setVersion(serviceEntity.getVersion() != null
...
//        service.setLabel(serviceEntity.getServiceDescription());
//        service.setId(serviceEntity.getServiceId());
